...
from abmt.trainers import ABMTTrainer, UsicTrainer, UsicTrainer_E
...
#4月24日 采用ProtoDataset+newDataset1组成新的NewDataset。 外加一个新的newDataset1，形成两个dataset来完成平衡训练+提升新数据的利用率。
...
def main():
    ...
    # 创建测试使用的dataloader
    test_loader_target = get_test_loader(dataset_target, args.height, args.width, 256, args.workers)#args.batch_size * 4
    ...
    for phase in range(start_phase, args.num_phase):
        ...
        tar_cluster_loader = get_test_loader(dataset_target, args.height, args.width, 256,
        ...
        for epoch in range(args.epochs):
            ...
            train_loader_examplar = get_train_loader(dataset_target, args.height, args.width,
            ...
            # Trainer
            trainer = UsicTrainer_E(model_cur=model_cur, model_ema=model_ema, model_ref=model_ref,
                                  old_class_num=old_class_num,new_class_num=args.num_clusters, alpha=args.alpha)


            train_loader_target.new_epoch()
            train_loader_examplar.new_epoch()  # 为了统一，可以在外面生成train_loader_examplar，在train()不使用
            trainer.train(phase, epoch, train_loader_target, train_loader_examplar, optimizer, cur_lamda,
